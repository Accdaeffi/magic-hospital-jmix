...
public class PatientAdministratorDiseasedBrowse extends StandardLookup<Patient> {
    ...
    public void onPatientsTableBuryAll(Action.ActionPerformedEvent event) {
        ...
        String result = corpses.stream().filter(c -> c.getPatientState() == PatientState.DISEASED).map(Patient::toString).collect(Collectors.joining("\n"));
        System.out.println(result);

        corpses.stream().filter(c -> c.getPatientState() == PatientState.DISEASED).forEach(c -> c.setPatientState(PatientState.BURIED));
    }
}